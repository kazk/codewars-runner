...
async function doSetup(opts, strategies) {
  ...
  if (opts.code || opts.solution) {
    ...
    await fs.outputFile(path.join(opts.dir, 'solution.txt'), opts.code || opts.solution);
  }
  ...
  if (opts.services.length != 0) {
    ...
    await Promise.all(opts.services.map(f => f(opts)));
  }
...
function runShell(opts, status) {
  return new Promise((resolve, reject) => {
    ...
    }, 'bash', [filename], {}, null, function(result) {
      opts.shell = result;
      resolve();
    });
  });
